...
function validar(){
	...
	let edita=/^[a-záéíóúñ]{4}[a-záéíóúñ \.]{7,33}[a-záéíóúñ\.]$/i; //Le pongo "/i" al final? Despues de $/?
	...
	var formato = obtenerFormato();
	var listaPaisesSeleccionados = obtenerIdiomas();
...
function obtenerIdiomas() {
	var listaPaisesSeleccionados = [];
	var options = document.primerFormulario.multiidioma.options;
	for (var i = 0; i < options.length; i++) {
		if (options[i].selected) {
			listaPaisesSeleccionados.push(options[i].value);
		}
	}

	return listaPaisesSeleccionados;
}

function obtenerFormato() {
	var formato = "";

	var options = document.primerFormulario.formato;

	for (var i = 0; i < options.length; i++) {
		if (options[i].checked) {
			formato = options[i].value;
			break;
		}
	}
...
function obtenerPaises() {
	...
	var options = document.primerFormulario.elements;

	for (var i = 0; i < options.length; i++) {
		if (options[i].type == 'checkbox' && options[i].checked) {
			paises += options[i].value + '\n';
		}
	}

...
function comprobarCantidadPorAlmacen($conn,$id_producto,$cantidad){

	//Recojo el array de la cookie lo recorro y compruebo que existe producto 
	
	$sql = "SELECT num_almacen from almacena where id_producto='$id_producto' and cantidad>='$cantidad' limit 1";
  ...
  try{
	...
	if(!empty ($resultado)){
		...
		return $resultado[0]['num_almacen'];
...
function comprarProducto($conn,$id_producto,$cantidad,$numAlmacenDisponible){
	//Tengo que hacer una query que reste en la tabla almacena la cantidad de producto comprado 
	//Y despues registre en la tabla compra la compra realizada 
	echo "a comprado algo";
